...
public class Ring : MonoBehaviour
{
    ...
    [Tooltip("当距离小于此值时，Ring会逃跑")]
    public float escapeDistance = 1.5f;
    [Tooltip("当距离大于此值时，Ring会随机移动（或向玩家靠近）")]
    ...
    /*
        void FixedUpdate()
        {
            ...
            {
                // 如果精灵默认面朝右
                sr.flipX = walkDirection.x > 0; // 向左移动 (<0)，翻转 (true)。向右移动 (>0)，不翻转 (false)。
            }
    ...
    */
    ...
    void FixedUpdate()
    {
        ...
        // 此时 walkDirection 由 RandomWalk 协程设置，不会每帧变化
        Vector2 targetDirection = walkDirection;
        animator.SetBool("is_walking", true); // 确保动画处于行走状态
        ...
        if (hit.collider != null)
        {
            // 撞墙处理：无论是逃跑还是随机行走，撞墙都必须换向
            // 尝试寻找一个切向方向 (Tangential direction)

            // 尝试左转90度
            Vector2 escapeTangentLeft = new Vector2(-targetDirection.y, targetDirection.x).normalized;
            // 尝试右转90度
            Vector2 escapeTangentRight = new Vector2(targetDirection.y, -targetDirection.x).normalized;

            // 优先选择一个没有碰撞的方向
            if (Physics2D.Raycast(rb.position, escapeTangentLeft, 0.2f, obstacleLayer).collider == null)
            {
                targetDirection = escapeTangentLeft;
                Debug.Log("Ring: 移动受阻，转向左侧切线。");
            }
            else if (Physics2D.Raycast(rb.position, escapeTangentRight, 0.2f, obstacleLayer).collider == null)
            {
                targetDirection = escapeTangentRight;
                Debug.Log("Ring: 移动受阻，转向右侧切线。");
            }
            else
            {
                // 如果两个切向都被阻挡，随机换向
                PickNewDirection();
                targetDirection = walkDirection;
                Debug.Log("Ring: 移动被包围，随机换向。");
            }

            // 【关键】：将新方向写回 walkDirection，确保在本次 Walk 阶段继续使用这个新方向
        ...
        // 移动
        Vector2 newPos = rb.position + targetDirection * moveSpeed * Time.fixedDeltaTime;
        rb.MovePosition(newPos);
    ...
    void DestroyObject()
    {
        ...
        {
            GameManager3.Instance.AddScore(10); // 每次消灭加 10 分
        }
        ...
        while (true)
        {
            ...
            if (playerTransform != null)
            {
                ...
                if (distance < escapeDistance)
                {
                    ...
                    walkDirection = -directionToPlayer;
                    Debug.Log("Ring: 决定逃跑。");
                }
                else
                {
